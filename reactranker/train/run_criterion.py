...
class run_test:
    ...
    def run_dis_model(self,
        ...
        file_name = os.path.split(saved_model_name)[1]
        path_name = os.path.split(saved_model_name)[0]
        # print(file_name)
        saved_data_name = 'pred_' + str(file_name.split('.')[0]) + '.csv'
        saved_data_path = os.path.join(path_name, saved_data_name)
    ...
    def run_mc_model(self,
        ...
        for i in range(sample_number):
            ...
            # save data path generation
            file_name = os.path.split(saved_model_name)[1]
            path_name = os.path.split(saved_model_name)[0]
            saved_data_name = 'pred_' + file_name.split('.')[0] + '_' + str(i) + '.csv'
            saved_data_path = os.path.join(path_name, saved_data_name)
    ...
    def run_ensemble_model(self, logger, smiles2graph_dic, test_data: pd.DataFrame, model: nn.Module, fold_number: int,
        ...
        for i in range(ensemble_number):
            ...
            path_name = self.path
            saved_data_name = 'pred_' + str(fold_number) + '_' + str(i) + '.csv'
            saved_data_path = os.path.join(path_name, saved_data_name)
